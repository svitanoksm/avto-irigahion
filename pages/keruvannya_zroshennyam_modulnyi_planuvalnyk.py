...
def load_schedule():
    ...
    try:
        ...
        # ВИПРАВЛЕННЯ:
        #
        # За замовчуванням gspread.get_all_records() читає
        # значення в режимі FORMATTED_VALUE — тобто так, як
        # число візуально відображається в Google Таблиці,
        # з урахуванням локалі (кома як десятковий роздільник)
        # та застосованого числового формату колонки.
        #
        # Крім того, get_all_records() сам намагається
        # "оцифрувати" (numericise) отриманий текст, і робить
        # це ДО того, як parse_duration() взагалі побачить
        # значення. Через це рядок "1,0" міг трактуватися як
        # число з роздільником розрядів і перетворюватися на
        # спотворене число ще на етапі читання.
        #
        # UNFORMATTED_VALUE повертає "сире" значення з таблиці:
        # для числових комірок — справжній float (1.0, 1.05),
        # без жодного впливу локалі чи формату відображення.
        # ------------------------------------------------------
...
def load_module_schedule():
    ...
    try:
        ...
        # ------------------------------------------------------
        # ТЕ САМЕ ВИПРАВЛЕННЯ, ЩО Й У load_schedule().
        #
        # Саме відсутність value_render_option="UNFORMATTED_VALUE"
        # тут була справжньою причиною бага з "тривалість, годин":
        #
        #   1,0   -> 10
        #   10    -> 100,0
        #   100   -> 1000,0
        #   100,05 -> 10000,5
        #
        # Оскільки без цього параметра gspread повертав
        # ВІДФОРМАТОВАНИЙ рядок (з урахуванням numberFormat
        # "0.0#", який виставляється в save_module_schedule()),
        # і одразу намагався перетворити його на число сам,
        # неправильно інтерпретуючи кому. Це зіпсоване число
        # потім записувалось назад у таблицю як "правильне",
        # і з кожним циклом читання/запису помилка множилась.
        # ------------------------------------------------------
        records = worksheet.get_all_records(
            value_render_option="UNFORMATTED_VALUE"
        )
...
def save_module_schedule(df):
    ...
    try:
        ...
        # ВАЖЛИВО: цей формат впливає лише на те, як число
        # ВИГЛЯДАЄ у самій Google Таблиці. Оскільки load_schedule()
        # і load_module_schedule() тепер читають дані через
        # value_render_option="UNFORMATTED_VALUE", цей формат
        # більше НЕ впливає на те, що зчитує застосунок.
        try:
