...
st.markdown(
    ...
    /* Відступ між тумблером і наступним елементом */
...
def get_tuya_settings():
    ...
    try:
        ...
        relay_group_device_id = safe_text(
...
st.caption(
    "Реле 1–4 працюють через один пристрій Tuya. "
    "Реле 5–7 мають окремі device_id."
)


# ============================================================
# ОТРИМУЄМО СТАНИ РЕЛЕ
# ============================================================
...
relay_columns = st.columns(
    2
)


for position, relay in enumerate(
    RELAY_CONFIG
):
    ...
    with relay_columns[
        ...
        with st.container(
            ...
            if not device_id:
                ...
                st.warning(
                    f"⚠️ Device ID для "
                    f"{name} ще не заданий."
                )

                st.caption(
                    "Реле поки не підключене."
                )
            ...
            if current_relay_state is True:

                st.success(
                    "🟢 Зараз УВІМКНЕНО"
                )

            elif current_relay_state is False:
            ...
            duration = st.number_input(
                ...
                help=(
                    "0 — без автоматичного "
                    "вимкнення. "
                    "Якщо вказати час, Tuya "
                    "автоматично вимкне реле "
            ...
            if (
                current_relay_state is not None
                and
                desired_state
                != current_relay_state
            ):
            ...
            if (
                ...
                duration > 0
                and
                current_relay_state is True
            ):

                if st.button(
                    "⏱️ Запустити / оновити таймер",
                    key=(
                        f"relay_timer_"
                        f"{number}"
                    ),
